...
public class OrdenService {
    ...
    public OrdenDTO buscarPorCarrito(Integer idCarrito) {
        ...
        Orden orden = ordenRepository.findByCarritoIdCarrito(idCarrito);
        if (orden == null) {
    ...
    public OrdenDTO guardar(Orden orden) {
        log.info("Creando nueva orden para carrito ID: {}", orden.getCarrito().getIdCarrito());
        Carrito carrito = carritoRepository.findById(orden.getCarrito().getIdCarrito())
                .orElseThrow(() -> {
                    log.error("Carrito no encontrado con ID: {}", orden.getCarrito().getIdCarrito());
                    return new RuntimeException("Carrito no encontrado con ID: " + orden.getCarrito().getIdCarrito());
                });
        orden.setCarrito(carrito);
        // Regla de negocio: registrar fecha de orden automáticamente
    ...
    private OrdenDTO convertirADTO(Orden orden) {
        ...
        dto.setIdCarrito(orden.getCarrito().getIdCarrito());
        return dto;
    }
}